...
import { Bell, Send, Loader2, Mail, Users, Clock, Check, Trash2 } from "lucide-react"
...
import { createBroadcastNotificationAction, deleteBroadcastNotificationAction } from "@/lib/actions/admin"
...
export default function AdminNotificationsPage() {
  ...
  const handleBroadcast = async () => {
    ...
    try {
      ...
      if ((form.sendMethod === "both" || form.sendMethod === "email") && userEmails.length > 0) {
        try {
      ...
      setForm({ title: "", message: "", type: "info", link: "", target: "all", sendMethod: "both" })
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        {/* Recent Notifications Feed (Moved to Left) */}
        <div className="lg:col-span-5 order-last lg:order-first">
          <Card className="bg-[#0f0f0f] border border-white/5 rounded-2xl p-6 space-y-4">
            ...
            <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
              {recentNotifs.map(n => (
                <div key={n.id} className="p-4 bg-white/5 border border-white/5 rounded-xl space-y-2 relative group transition-all hover:bg-white/10">
                  <Button 
                    onClick={() => handleDelete(n.id)}
                    className="absolute top-2 right-2 h-7 w-7 p-0 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white rounded-lg"
                    title="Delete Notification from all students"
                  >
                  ...
                  <p className="text-[10px] text-slate-500 font-medium leading-relaxed whitespace-pre-wrap">{n.message}</p>
          ...
          <Card className="bg-[#0f0f0f] border border-white/5 rounded-2xl p-8 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5 col-span-2 sm:col-span-1">
                <label className="text-[9px] font-black uppercase text-slate-500 tracking-widest">Sending Method</label>
                <Select value={form.sendMethod} onValueChange={v => setForm({ ...form, sendMethod: v })}>
                  ...
                  <SelectContent className="bg-[#0f0f0f] border-white/10 text-white text-xs">
                    ...
                    <SelectItem value="inapp">In-App Notification Only</SelectItem>
                    <SelectItem value="email">Email Blast Only</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            ...
            <div className="p-4 bg-[#00FF88]/5 border border-[#00FF88]/10 rounded-xl flex items-center gap-3">
              ...
              <p className="text-xs font-bold text-[#00FF88]">
                This broadcast will reach <span className="font-black">{userCount} students</span> 
                {form.sendMethod === 'both' ? ' via in-app & email.' : form.sendMethod === 'inapp' ? ' via in-app only.' : ' via email only.'}
              </p>
            </div>
